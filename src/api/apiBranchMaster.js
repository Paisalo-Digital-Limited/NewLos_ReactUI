...
export const deleteBranchMaster = async (id, params) => {
  try {
    const response = await apiClient.post(`https://apiuat.paisalo.in:4015/admin/api/Masters/DeleteBranchMaster?Id=${id}`, params, {});
    return response.data;
  } catch (error) {
    console.error('API Error:', error);
    Swal.fire({
...
export const updateBranchMaster = async (payload) => {
  try {
    ...
    throw new Error(error.response?.data?.message || 'Failed to update branch master.');
  }
};
