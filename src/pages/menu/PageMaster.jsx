  ...
  const handleMenuChange = (event) => {
    ...
    if (selectedMenuItem) {
      ...
      if (
        menuListData.find(
          (submenuItem) =>
            submenuItem.id === subparentId &&
            submenuItem.parentId === selectedMenuItem.id
        )
      ) {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      if (response.data.statuscode === 200) {
        ...
        Swal.fire({
          title: "Error",
          text: response.data.message || "Failed to add item.",
          icon: "error",
        });
      }
    } catch (error) {
      Swal.fire({
        icon: "error",
        title: "Error!",
        text: "An error occurred while inserting menu data.",
      });
    }
  };
    ...
    <>
        ...
        >
          ...
          <Grid item xs={12} sm={3} md={2}>
            <FormControl fullWidth error={!!errors.subMenu}>
              ...
              {errors.subMenu && (
                <FormHelperText>{errors.subMenu}</FormHelperText>
              )}
            </FormControl>
          </Grid>
          <Grid item xs={12} sm={3} md={2}>
      ...
      >
        <Table>
          ...
          <TableBody>
          {(rowsPerPage > 0
                ? menuListData.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage)
                : menuListData
              ).map((item, index) => (
            ...
            <FormControl fullWidth error={!!errors.subMenu}>
              ...
              <Select
                ...
                onChange={(e) => {
                  ...
                  const selectedSubMenu = menuListData.find(
                    (submenuItem) => submenuItem.mainid === selectedMainId
                  ); // Find by mainid
                  if (selectedSubMenu) {
              ...
              >
                {menuListData.length > 0 && menu ? (
                  menuListData
                    ...
                    .map((submenuItem) => (
                      <MenuItem
                        key={submenuItem.mainid}
                        value={submenuItem.mainid}
                      >
                        {" "}
                        {/* Use mainid as value */}
                        {submenuItem.title}
